#!/usr/bin/env python3
"""
Grades the Erfindungsbeschreibung process quality using OpenAI.
...
def main():
    ...
    prompt = """You are a harsh, critical quality assessor. Your job is to GRADE the "Erfindungsbeschreibung" process (a German guide for inventors on documenting their invention for TTO forms) using the USER PERSONA as your lens. Be genuinely critical – do not sugarcoat. If something is weak, say so. If the process fails the persona in some way, call it out.
...
### 1. Persona Fit
- Does the process actually serve the "Erfinder:in, die erklären kann, aber nicht dokumentieren"?
...
- Are the examples from TUM, HM, JMU, THI used effectively?

### 2. Completeness
- Are there gaps in the process? Steps missing? Transitions unclear?
- Does it cover the full structure (Kern, Stand der Technik, Lösung, Struktur, Fehler)?
- What could a user get stuck on with no guidance?

### 3. Clarity & Actionability
- Are the instructions concrete enough for someone who struggles to document?
- Where is the language vague or ambiguous?
- Do the "Tipp" sections actually help, or are they generic?
- Is the TUM rule ("what one must do so that it works") clearly conveyed?
...
# Erfindungsbeschreibung: Quality Grade

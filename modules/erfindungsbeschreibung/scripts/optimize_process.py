#!/usr/bin/env python3
"""
AI Process Optimization Machine for Erfindungsbeschreibung

Uses PROCESS-SCAFFOLD.md to generate optimized versions of the Erfindungsbeschreibung process.
- Phase 1: Generate optimized variant(s)
- Phase 2: Review and refine the output

Uses GPT-4o (or gpt-5 if available).
"""
...
def call_ai(client, messages, model=MODEL):
    """Call OpenAI, fallback if model unavailable."""
    try:
...
def phase1_generate(client, inputs):
    ...
    prompt = """You are a process design expert. Your task is to create an OPTIMIZED version of the "Erfindungsbeschreibung" process (Invention Documentation Guide for TTO forms).

## Inputs
- PROCESS-SCAFFOLD: Defines the structure (Titel, Ziel, So gehst du vor, Tipp, Fazit, Nächster Schritt)
- USER-PERSONA: Target user = "Erfinder:in, die erklären kann, aber nicht dokumentieren"
...
5. **Add elements** – e.g., clearer examples from TUM, HM, JMU, THI; more concrete Tipps

## Constraints
- MUST follow the scaffold structure for each step
- MUST serve the persona: practical, with real examples
...
---
FILE: 01-Kern-der-Neuerung.md
---
[full markdown content of step 1]
---
... and so on for all steps (00 through 06).

Filenames: 00-Einleitung.md, 01-Kern-der-Neuerung.md, 02-Stand-der-Technik.md, 03-Technische-Loesung.md, 04-Struktur-und-Form.md, 05-Typische-Fehler-vermeiden.md, 06-Ergebnis.md
...
def phase2_review_and_refine(client, phase1_output, inputs):
    ...
    prompt = """You are a critical reviewer. The following is an AI-generated OPTIMIZED version of the Erfindungsbeschreibung process (from Phase 1).

Your task: REVIEW it critically, then REFINE it.

## Review Checklist
1. Does it follow the scaffold structure consistently?
2. Does it actually serve the persona (inventor who can explain but not document)?
3. Are the step transitions clear? Input-output handoffs explicit?
4. Are the examples (TUM, HM, JMU, THI) used effectively?
5. Is the TUM rule ("what one must do so that it works") clearly conveyed?

## Your Task
1. First output a REVIEW section: What's good? What's still weak?
2. Then output the REFINED process: Apply your improvements.
